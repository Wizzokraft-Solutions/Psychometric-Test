...
- No employee list and no Emp No./boss/tenure/role. People enter only **Name, Mobile Number (10-digit Indian, stored normalized), Date of Birth, Designation, Department**. **Retakes are allowed (2026-09-18):** there is no once-per-day limit — every attempt is stored as its own row and all of them show in admin. (`survey_person_key` still normalises names case-/space-insensitively, for grouping.)
- Admin opens/closes days (`set_active_day`); the day is never chosen by the browser.
- DB: `supabase/survey.sql` (tables `survey_questions`, `survey_settings`, `survey_submissions` + RPCs `get_survey`, `submit_survey`, `get_survey_admin_data`, `set_active_day`). Retakes migration for an already-deployed DB: paste `supabase/allow-retakes.sql` into the SQL Editor (drops the `survey_submissions_person_day` unique index and `has_submitted_survey`); `supabase/survey.sql` already matches. Questions seed: `node scripts/parse-survey.mjs` → `content/survey-seed.sql`, paste into SQL Editor.
